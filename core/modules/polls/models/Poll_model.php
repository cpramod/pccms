...
class Poll_model extends My_Model {
	...
	{
		$this->config->load('polls/table');
		$this->polls_table = config('tbl_polls');
		$this->options_table = config('tbl_option');
		$this->votes_table = config('tbl_votes');
	}
	...
	public function has_previously_voted($poll_id)
	{
		$this->db->from($this->options_table)
			->join($this->votes_table, $this->votes_table.'.option_id = '.$this->options_table.'.option_id')
			->where($this->options_table.'.poll_id', $poll_id)
			->where($this->votes_table.'.ip_address', $this->input->ip_address());
	...
	public function has_previously_voted_within($interval, $poll_id)
	{
		$this->db->from($this->options_table)
			->join($this->votes_table, $this->votes_table .'.option_id = '.$this->options_table .'.option_id')
			->where($this->options_table.'.poll_id', $poll_id)
			->where($this->votes_table .'.timestamp >', time() - $interval)
			->where($this->votes_table .'.ip_address', $this->input->ip_address());
