...
class Pages extends My_Controller
{
    ...
    {
        //Get Articles
        $data['pages'] = $this->pages_m->get_articles('id','DESC', $this->limit, $offset)->result();
        $total = count($this->pages_m->get_articles('id', 'DESC')->result());
      ...
      public function category_edit($id){
            ...
            if ($this->input->post()) {
                ...
                if (($post_data['slug'] != '') && ($post_data['slug'] != $data->slug)) {
                    $this->form_validation->set_rules('slug', 'URL title', 'required|alpha_dash|is_unique[categories.slug]');
                    $build_slug = false;
                }
