...
class Auth extends Front_Controller {
	...
	public function login()
	{
		...
		if ($this->ion_auth->logged_in()) {
			echo json_encode(array('message' => '', 'status' => 1, 'referrer' => site_url($referrer)));
			exit;
		}
	...
	public function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template
					...
					->set('class','page')
					->set_breadcrumb('Deactivate user','')
					->set('menu','deactivate')
					->build('deactivate_user', $this->data);
		}
		else
		{
	...
	public function do_login($email, $password)
	{
		...
		$login = $this->ion_auth->login($email, $password, $remember = 0);

		if ($login) {
			$user_id = $this->session->userdata('user_id');
			echo json_encode(array('message' => $this->ion_auth->messages(), 'status' => 1));
			exit;
       ...
       if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				if ($this->input->post('avatar')) {
					$base64 = $this->input->post('avatar');
					$filename = 'profile-' . $this->session->userdata('user_id') . '.jpg';
					$output = UPLOAD_PATH . $filename;
					base64_to_jpeg($base64, $output);
					$upload_id = $this->ion_auth_model->insert(config('tbl_upload'), array('title' => $filename, 'link' => $filename));
					$data['avatar'] = $upload_id;
				}
			...
			// check to see if we are updating the user
                if($this->ion_auth->update($user->id, $data))
			    {
					...
					$this->load->model('profile/profile_m', 'profile_m', true);
					...
					$user_meta = array(
						...
						'language' => serialize($this->input->post('language')),
						'qualification' => $this->input->post('qualification'),
						'user_id' => $id,
						'job_category' => serialize($this->input->post('job_category')),
						'latitude' => $this->input->post('latitude'),
						'longitude' => $this->input->post('longitude')

					);

					$this->profile_m->delete(config('user_meta'), array('user_id' => $id));

					if ($this->profile_m->insert(config('user_meta'), $user_meta)) {
...
//        ->build('auth/edit_user', $this->data);
    ...
    public function do_upload()
    {
        ...
        {
            return array('error'=>1, 'msg'=>$this->upload->display_errors());

        }
        else
        {
            return array('error'=>0, 'msg'=>$this->upload->data());
        }
    }
